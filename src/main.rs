...
fn main() {
    let matches = App::new("rusty-ping")
                    .version("1.0")
                    .about("Ping CLI application built for Cloudflare's systems programming challenge")
    ...
    match ipaddr {
        ...
        IpAddr::V6(ip) => {
            // Pinging IPv6 using the Pnet rust crate does not seem to be properly supported.
            // Although the code is written to work, this will throw an OS error.
            v6_ping(ip, ttl);
        }
    }
            ...
            if let Some((packet, addr)) = iter.next_with_timeout(timeout).unwrap() {
                ...
                if icmp_packet.get_sequence_number() == sequence_number {
                    if icmp_packet.get_icmp_type() == IcmpTypes::EchoReply {
                        ...
                        // Attempt to reach the destination by increasing the time to live
                        ttl += if ttl == 255 { 0 } else { 1 };
                    }
                    else {
                ...
                // Packet may have time exceeded without a return message. Increase time to live to be safe.
                ttl += if ttl == 255 { 0 } else { 1 }; 
                break;
            }
        }
...
fn build_v4_packet<'x>(ip_buffer: &'x mut [u8], icmp_buffer: &'x mut [u8], dest: Ipv4Addr, sequence_number: u16, ttl:u8) -> MutableIpv4Packet<'x> {
    ...
    ip_packet.set_version(4);           // Ipv4's version number is 4
    ip_packet.set_header_length(5);     // 20 bytes
    ip_packet.set_total_length(28);     // Size of ipv4 (20) + size of icmp header (8)
    ip_packet.set_identification(44);   // Magic number
...
/**
 * This is the attempt to implement the Ipv6 version of the ping application.
 * The theory and code setup should theoretically work, however, it would seem
 * that the pnet crate used does not properly support ipv6 as attempting to send
 * a ipv6 packet results in a OS error with code 97 ("Address familty not supported 
 * by protocol").
 */
